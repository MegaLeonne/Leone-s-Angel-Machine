# Angel Machine - Master Index

> [!NOTE]
> This is a static manifest for automated tool discovery. [Return to Home](https://megaleonne.github.io/Leone-s-Angel-Machine/)

## 🎆 Start Here
_Essential entry points_

- [README](file:///README.md) (ID: `#README`)
- [The Head Archetype](file:///docs/archetypes/The-Head-Archetype.md) (ID: `#The-Head-Archetype`)
- [Guiding Prompt](file:///docs/philosophy/Guiding-Prompt.md) (ID: `#Guiding-Prompt`)

## 🌀 The Reservoir
- [KHAOS NOTES](file:///docs/khaos/KHAOS_NOTES.md) (ID: `#KHAOS_NOTES`)
- [Resonance-Void-001](file:///docs/khaos/Resonance-Void-001.md) (ID: `#Resonance-Void-001`)
- [Lexus Echoes](file:///docs/khaos/Lexus-Echoes-001.md) (ID: `#Lexus-Echoes-001`)
- [Resonance-Void-003](file:///docs/khaos/Resonance-Void-003.md) (ID: `#Resonance-Void-003`)

## ⚖️ Laws of Khaos
_Governing principles and primal forces_

- [KHAOS](file:///docs/archive/ash-layers/fragments/KHAOS.md) (ID: `#KHAOS`)
- [The Law](file:///docs/philosophy/philosophy-the-law.md) (ID: `#philosophy-the-law`)
- [The Law (Mirror)](file:///docs/archive/ash-layers/cryptic/}}[[THE LAW.md) (ID: `#}}[[THE-LAW`)
- [There Are No Rules](file:///docs/archive/ash-layers/fragments/There Are No Rules.md) (ID: `#There-Are-No-Rules`)
- [The Rule Of The Swamp](file:///docs/archive/ash-layers/fragments/The Rule OF The Swamp..md) (ID: `#The-Rule-OF-The-Swamp.`)
- [Walk Alongside Khaos](file:///docs/archive/ash-layers/fragments/WalkAlongSideKhaos.md) (ID: `#WalkAlongSideKhaos`)

## 💭 Philosophy
_Foundational texts and cosmology_

- [Passion](file:///docs/philosophy/Passion.md) (ID: `#Passion`)
- [Patience](file:///docs/philosophy/Patience.md) (ID: `#Patience`)
- [Prophecy](file:///docs/philosophy/philosophy-prophecy.md) (ID: `#philosophy-prophecy`)
- [The Material](file:///docs/philosophy/The Material.md) (ID: `#The-Material`)
- [Amnesia](file:///docs/philosophy/Amnesia.md) (ID: `#Amnesia`)
- [The Void](file:///docs/philosophy/The Void.md) (ID: `#The-Void`)
- [The Abyss](file:///docs/philosophy/The-Abyss.md) (ID: `#The-Abyss`)
- [The Ticks](file:///docs/philosophy/The-Ticks.md) (ID: `#The-Ticks`)
- [Void Tongue](file:///docs/philosophy/Void-Tongue.md) (ID: `#Void-Tongue`)
- [Khaos](file:///docs/philosophy/philosophy-khaos.md) (ID: `#philosophy-khaos`)
- [Providence](file:///docs/philosophy/philosophy-providence.md) (ID: `#philosophy-providence`)

## 🎭 Archetypes
_Mythic figures and divine principles_

- [The Head / Witch Queen](file:///docs/archetypes/The-Head-Archetype.md) (ID: `#The-Head-Archetype`)
- [The Heart Archetype](file:///docs/archetypes/The-Heart-Archetype.md) (ID: `#The-Heart-Archetype`)
- [She Who Transcends](file:///docs/archetypes/She-Who-Transcends.md) (ID: `#She-Who-Transcends`)
- [Michael, The KlockWork Angel](file:///docs/archetypes/Angels/Michael, The KlockWork Angel.md) (ID: `#Michael-The-KlockWork-Angel`)
- [Osiris, Arch Angel Of Omen](file:///docs/archetypes/Angels/Osiris, Arch Angel Of Omen.md) (ID: `#Osiris-Arch-Angel-Of-Omen`)
- [Taylor, Devil of Desire](file:///docs/archetypes/Devils/Taylor, Devil of Desire.md) (ID: `#Taylor-Devil-of-Desire`)
- [The Blind](file:///docs/archetypes/Devils/The Blind.md) (ID: `#The-Blind`)
- [He Who Remains](file:///docs/archetypes/He-Who-Remains.md) (ID: `#He-Who-Remains`)
- [Shadow-Demon-Providence](file:///docs/archetypes/Composite/Shadow-Demon-Providence.md) (ID: `#Shadow-Demon-Providence`)

## 📼 Historical Logs
- [The Heart's Reprise](file:///docs/the-seventh-borough/mechanics/The-Hearts-Reprise.md) (ID: `#The-Hearts-Reprise`)
- [Event Log 009](file:///docs/the-seventh-borough/archives/Event-Log-009-The-Hearts-Reprise.md) (ID: `#Event-Log-009-The-Hearts-Reprise`)
- [The Seventh Call](file:///docs/rituals/ritual-seventh-call.md) (ID: `#ritual-seventh-call`)
- [ASH REMNANTS](file:///docs/archive/REMNANTS.md) (ID: `#REMNANTS`)

## 🔮 Rituals
_Practical invocations and spells_

- [Counterspell Playlist](file:///docs/rituals/Counterspell-Playlist.md) (ID: `#Counterspell-Playlist`)
- [Angel Numbers](file:///docs/rituals/Angel Numbers.md) (ID: `#Angel-Numbers`)
- [Deck of Fate](file:///docs/rituals/Deck of Fate.md) (ID: `#Deck-of-Fate`)
- [The Spells It Makes](file:///docs/rituals/The Spells It Makes.md) (ID: `#The-Spells-It-Makes`)
- [King's Gambit](file:///docs/khaos/spell-kings-gambit.md) (ID: `#spell-kings-gambit`)
- [Witch's Ceiling Fall](file:///docs/rituals/ritual-witchs-ceiling-fall.md) (ID: `#ritual-witchs-ceiling-fall`)

## 🏛️ The Seventh Borough
_Canon framework and workflow_

- [Seventh Borough Canon](file:///docs/the-seventh-borough/CANON.md) (ID: `#CANON`)
- [Curation Threshold](file:///docs/the-seventh-borough/mechanics/ritual-elevation-protocol.md) (ID: `#ritual-elevation-protocol`)
- [Tagging System](file:///docs/the-seventh-borough/TAGS.md) (ID: `#TAGS`)

